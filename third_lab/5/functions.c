...
return_code is_correct_input(Student student) {
    if (strlen(student.name) == 0 || strlen(student.last_name) == 0 || strlen(student.group) == 0) {
        return INVALID_INPUT;
    }
    for (size_t i = 0; i < strlen(student.name); ++i) {
        if (!isalpha((char)student.name[i])) {
            return INVALID_INPUT;
        }
    }
    for (size_t i = 0; i < strlen(student.last_name); ++i) {
        if (!isalpha((char)student.last_name[i])) {
            return INVALID_INPUT;
        }
    }
    for (size_t i = 0; i < strlen((char*)student.marks); ++i) {
        if (!isdigit(student.marks[i])) {
            return INVALID_INPUT;
        }
    }
    return OK;
...
return_code upload_info(Student** output_info, int* size, FILE* input_file) {
    ...
    *size = 0;
    ...
    while ((reading_cnt = fscanf(input_file, "%u%s%s%s%s\n",
                                 &students[*size].id, students[*size].name,
                                  students[*size].last_name, students[*size].group,
                                   students[*size].marks)) == 5) {
        if (is_correct_input(students[*size]) != OK) {
