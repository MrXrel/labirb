...
enum return_code zeckendorf_to_int(const char* zeckendorf_repr, unsigned int* result) {
...
enum return_code is_valid_roman(const char *number, int* is_valid) {
    if (!number || !is_valid) {
        return BAD_POINTER_ERROR;
    }
    regex_t regex;
...
enum return_code roman_to_int(const char *roman, int *result) {
    if (!roman || !result) {
        return BAD_POINTER_ERROR;
    }
    int total = 0;
...
enum return_code handle_custom_scan_flags(
    void* stream, enum TYPE_OF_SCAN type_of_scan, const char* pos,
...
enum return_code general_scanf(void* stream, enum TYPE_OF_SCAN type_of_scan, const char* format, va_list* args) {
    ...
    while (*pos) {
        if (*pos == '%') {
            ...
            buffer[buffer_pos] = *pos;
            buffer[buffer_pos + 1] = '\0'; // Завершаем строку
