...
enum return_code print_to_stream_reversed(void* stream, TYPE_OF_PRINT type, const char* reversed_str, int ind_of_end) {
    if (!reversed_str) {
...
enum return_code convert_to_decimal(const char* num_repr_in_base, int end_of_num_repr, format_for_char format_char,int base, int* decimal_result) {
    *decimal_result = 0;
    int value;
    int sign = 1;
...
enum return_code convert_decimal_to_base(int x, int base, format_for_char format_char, char* representation, int* ind) {
...
enum return_code generate_fibonacci(unsigned int n, unsigned int* size, unsigned int** fib_result) {
    if (!size || ! fib_result) {
        return BAD_POINTER_ERROR;
    }
    if (n < 1) {
...
enum return_code print_number_in_base_to_decimal(void* stream, TYPE_OF_PRINT type_of_print, format_for_char format_char, char const* num_repr, int base) {
    int decimal_value = 0;
...
enum return_code general_printf(void* stream, TYPE_OF_PRINT type_of_print, const char* format, va_list* args) {
...
enum return_code overfprintf(FILE* stream, char const* format, ...) {
    va_list args;
    va_start(args, format);
